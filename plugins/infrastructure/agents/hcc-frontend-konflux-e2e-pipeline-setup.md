...
3. Check for existing Playwright tests
   ...
   - **CRITICAL:** Verify Playwright is installed in package.json:
     ...
     - **CRITICAL:** Verify Playwright config uses sequential execution:
       ...
       - This prevents race conditions and flaky tests in CI environments
     - **CRITICAL:** Verify HCC_ENV_URL is NOT used in test config:
       - Playwright config should use `PLAYWRIGHT_BASE_URL` or hardcoded URLs
   ...
   - **CRITICAL:** Install the shared authentication package:
     - Check `devDependencies` for `@redhat-cloud-services/playwright-test-auth`
     - If missing, add it: `npm install -D @redhat-cloud-services/playwright-test-auth`
     - This package provides reusable login/auth code with the following benefits:
     ...
     - **CRITICAL VERSION ALIGNMENT**: Three versions must match:
       1. The `@playwright/test` version in your repo's package.json
       2. The playwright version that `@redhat-cloud-services/playwright-test-auth` depends on
       3. The `e2e-playwright-image` parameter in your pipeline YAML
       - Check the auth package's required Playwright version: `npm info @redhat-cloud-services/playwright-test-auth peerDependencies`
   ...
   - **CRITICAL:** Verify test files do NOT reference HCC_ENV_URL:
     - Search test files for "HCC_ENV_URL" - should NOT be found
     - Tests should rely on baseURL from config or hardcoded URLs
     - HCC_ENV_URL is managed by pipeline infrastructure, not test code
```
...
- You skip steps 10c (Generate ExternalSecret YAML) and 10d (Submit ExternalSecret to konflux-release-data)
...
- The pipeline configuration in step 10e remains the same (reference the secret name)

**10c. Generate ExternalSecret YAML (skip if using direct Kubernetes secrets)**

**Note:** If you created a secret directly via the Konflux UI in the alternative approach above, skip to step 10e. This step is only for the ExternalSecret/Vault approach.
...
**10d. Submit ExternalSecret to konflux-release-data (skip if using direct Kubernetes secrets)**
...
Ensure the E2E pipeline references the correct secret name. The configuration is the same regardless of which approach you used (ExternalSecret or direct Kubernetes secret):
...
- The secret name must be `<app-name>-credentials-secret` (as created in step 10b alternative approach)
- All 4 secret keys are required: e2e-user, e2e-password, e2e-hcc-env-url, e2e-stage-actual-hostname
- The pipeline automatically mounts the secret into the test container
...
**Step 12a: Submit and Merge konflux-release-data PR First (or Create Secret via UI)**
...
5. Create the secret via Konflux UI (as described in step 10b alternative approach) with name `<app-name>-credentials-secret`
...
Solution:
1. **First, check the auth package's required Playwright version**:
   ```bash
   npm info @redhat-cloud-services/playwright-test-auth peerDependencies
   # This will show the required @playwright/test version
   ```

2. Install Playwright and the shared authentication package as dev dependencies with matching versions:
   ```bash
   # Example: If auth package requires Playwright 1.40.0
   npm install -D @playwright/test@1.40.0 @redhat-cloud-services/playwright-test-auth
   ```

3. Verify package.json includes both packages with aligned versions and the test script:
   ```json
   {
     "scripts": {
...
4. **Update pipeline to use matching Docker image**:
   ```yaml
   - name: e2e-playwright-image
...
5. **CRITICAL: Configure playwright.config.ts with shared authentication**:
   ...
   Key requirements for all new test suites:
   ```typescript
   ...
   export default defineConfig({
     ...
     /* CRITICAL: Global setup for authentication using shared package */
     globalSetup: require.resolve('@redhat-cloud-services/playwright-test-auth/global-setup'),

     /* Shared settings for all the projects below */
     use: {
       /* Base URL to use in actions like `await page.goto('/')` */
       /* CRITICAL: Use PLAYWRIGHT_BASE_URL, NOT HCC_ENV_URL */
       /* HCC_ENV_URL is for pipeline infrastructure only */
       baseURL: process.env.PLAYWRIGHT_BASE_URL || 'https://stage.foo.redhat.com:1337',

       /* Collect trace when retrying the failed test */
       trace: 'on-first-retry',

       /* CRITICAL: Ignore HTTPS errors for stage environment */
       ignoreHTTPSErrors: true,

       /* CRITICAL: Reuse authentication state from global setup */
   ...
   **Benefits of using @redhat-cloud-services/playwright-test-auth:**
   - **Deduplication**: Eliminates duplicate login/auth code across test suites
   - **Session sharing**: Authenticated session state is shared across all specs and tests via `storageState`
   - **Consistency**: Provides uniform authentication approach across all E2E tests
   - **Performance**: Reduces test execution time by reusing authentication (login once, test many times)
   ...
   **Important configuration settings:**
   ...
   - `storageState: 'playwright/.auth/user.json'` - Shares auth state across tests
   - `ignoreHTTPSErrors: true` - Required for stage environment testing
   - `baseURL: process.env.PLAYWRIGHT_BASE_URL || 'https://stage.foo.redhat.com:1337'` - Uses pipeline-provided URL or fallback

   **Note:** The authentication package automatically reads credentials from environment variables:
   - `E2E_USER` - Test automation username (injected from e2e-credentials-secret)
   - `E2E_PASSWORD` - Test automation password (injected from e2e-credentials-secret)

   These credentials are provided by the pipeline through the ExternalSecret configuration (see Step 10).
...
6. Commit the changes and re-run tests
...
When you upgrade Playwright, you must:
- Check if the auth package supports the new version
- Update package.json to match
- Update the pipeline's e2e-playwright-image parameter
...
**Common Issue 12: Flaky Tests Due to Parallel Execution**
...
or
```
Intermittent failures with "element not found" or "timeout" errors
```
or
```
Tests fail with race conditions or data conflicts
```

Causes:
- Playwright configured to run tests in parallel (`workers > 1`)
- Tests share state or resources without proper isolation
- Multiple tests modifying the same data simultaneously
- Browser contexts not properly isolated between parallel tests

Solution:
1. **Verify Playwright configuration enforces sequential execution**:
   ```typescript
...
2. **Check for parallel execution configuration**:
   ```bash
   # Look for problematic settings in playwright.config.ts/js
   grep -E "workers|fullyParallel" playwright.config.*
...
3. **Update configuration if needed**:
   - Set `workers: 1` to force sequential test execution
   - Set `fullyParallel: false` to prevent parallel execution within test files
   - Remove any `test.describe.parallel()` blocks that force parallelism
   - Commit the changes and re-run tests

4. **Test locally before pushing**:
   ```bash
   # Run tests to verify sequential execution
   npm run test:playwright

   # Tests should run one at a time, not in parallel
   ```

**Why Sequential Execution Matters:**
- Prevents race conditions when tests modify shared data
- Reduces flaky tests caused by resource contention
- Makes debugging easier (tests run in predictable order)
- Ensures consistent behavior between local and CI environments
- Parallel execution can be enabled later once tests are properly isolated

**CRITICAL:** Always configure sequential execution first. Only enable parallel execution after verifying all tests are properly isolated and pass consistently.

**Common Issue 13: HCC_ENV_URL Used in Test Code or Config**

Symptoms:
```
Tests reference HCC_ENV_URL directly
```
or
```
playwright.config.ts uses process.env.HCC_ENV_URL
```
or
```
Tests fail with "undefined" baseURL errors
```

Causes:
- HCC_ENV_URL used in playwright.config.ts instead of PLAYWRIGHT_BASE_URL
- Test files directly reference process.env.HCC_ENV_URL
- Confusion about which environment variables are for tests vs infrastructure
- Copying configuration from pipeline YAML into test code

Problem:
HCC_ENV_URL is strictly for **pipeline infrastructure** (pipeline YAML and sidecar scripts). Using it in test code:
- Violates separation of concerns (infrastructure vs test code)
- Makes tests dependent on pipeline-specific environment variables
- Prevents tests from running locally without pipeline infrastructure
- Creates confusion about environment variable boundaries
- May expose infrastructure details to test code unnecessarily

Solution:
1. **Check for HCC_ENV_URL in test configuration**:
   ```bash
   # Search playwright config files
   grep -E "HCC_ENV_URL" playwright.config.*

   # Should return NOTHING - if found, this is an error
   ```

2. **Check for HCC_ENV_URL in test files**:
   ```bash
   # Search all test files
   grep -r "HCC_ENV_URL" playwright/

   # Should return NOTHING - if found, this is an error
   ```

3. **Fix playwright.config.ts to use PLAYWRIGHT_BASE_URL**:
   ```typescript
   // WRONG - Don't do this:
   export default defineConfig({
     use: {
       baseURL: process.env.HCC_ENV_URL,  // ❌ WRONG - infrastructure variable
     },
   });

   // CORRECT - Use PLAYWRIGHT_BASE_URL:
   export default defineConfig({
     use: {
       baseURL: process.env.PLAYWRIGHT_BASE_URL || 'https://stage.foo.redhat.com:1337',  // ✅ CORRECT
     },
   });
   ```

4. **Fix test files to use baseURL from config**:
   ```typescript
   // WRONG - Don't do this:
   test('my test', async ({ page }) => {
     await page.goto(process.env.HCC_ENV_URL + '/apps/myapp');  // ❌ WRONG
   });

   // CORRECT - Use relative URLs (baseURL from config):
   test('my test', async ({ page }) => {
     await page.goto('/apps/myapp');  // ✅ CORRECT - uses baseURL from config
   });
   ```

5. **Verify separation of concerns**:
   ```bash
   # Verify test code is clean
   grep -r "HCC_ENV_URL" playwright/ playwright.config.*

   # Should return no results
   ```

**Where HCC_ENV_URL SHOULD be used:**
- ✅ Pipeline YAML files (`.tekton/*.yaml`)
- ✅ Sidecar container scripts
- ✅ Proxy ConfigMap configurations
- ✅ Pipeline infrastructure scripts

**Where HCC_ENV_URL should NOT be used:**
- ❌ playwright.config.ts or playwright.config.js
- ❌ Test files (*.spec.ts, *.test.ts)
- ❌ Test helper/utility files
- ❌ package.json scripts

**CRITICAL:** Maintain strict separation between infrastructure variables (HCC_ENV_URL) and test variables (PLAYWRIGHT_BASE_URL). This ensures tests can run locally and aren't coupled to pipeline infrastructure.
...
1. Assessment Phase:
   ...
   - **CRITICAL**: Verify three-way Playwright version alignment:
     1. Check `@redhat-cloud-services/playwright-test-auth` required Playwright version:
        ```bash
        npm info @redhat-cloud-services/playwright-test-auth peerDependencies
        ```
     2. Ensure package.json `@playwright/test` matches the auth package requirement
     3. Note the version number (e.g., "1.40.0") - you'll use this for the pipeline's `e2e-playwright-image` parameter
...
### Anti-Pattern 9: Using HCC_ENV_URL in Test Code or Config
**DON'T:**
```typescript
// playwright.config.ts
export default defineConfig({
...
export default defineConfig({
  use: {
    // Use PLAYWRIGHT_BASE_URL for test configuration
    baseURL: process.env.PLAYWRIGHT_BASE_URL || 'https://stage.foo.redhat.com:1337',  // ✅ CORRECT
  },
});
...
// test file
test('my test', async ({ page }) => {
  // Use relative URLs - baseURL comes from config
  await page.goto('/apps/myapp');  // ✅ CORRECT
});
```

**Why this matters:**
- HCC_ENV_URL is for pipeline infrastructure (sidecars, proxies) only
- PLAYWRIGHT_BASE_URL is for test code and configuration
- Mixing these creates coupling between tests and infrastructure
- Tests should be runnable locally without pipeline infrastructure
- Proper separation makes tests more maintainable and portable
- Validates with: `grep -r "HCC_ENV_URL" playwright/ playwright.config.*` (should return nothing)
...
- [ ] Playwright dependencies installed:
  ...
  - [ ] **CRITICAL**: `@redhat-cloud-services/playwright-test-auth` in package.json devDependencies
  - [ ] **CRITICAL THREE-WAY VERSION ALIGNMENT**: All three versions match:
    - [ ] `@playwright/test` version in package.json (e.g., "^1.40.0")
    - [ ] Playwright version required by `@redhat-cloud-services/playwright-test-auth` (check with `npm info @redhat-cloud-services/playwright-test-auth peerDependencies`)
    - [ ] `e2e-playwright-image` parameter in pipeline YAML (e.g., "mcr.microsoft.com/playwright:v1.40.0-jammy")
  - [ ] `playwright.config.ts` or `playwright.config.js` exists
  - [ ] **CRITICAL**: Playwright config uses sequential execution in CI (`workers: process.env.CI ? 1 : undefined`)
  - [ ] **CRITICAL**: playwright.config.ts uses shared auth setup (`globalSetup: require.resolve('@redhat-cloud-services/playwright-test-auth/global-setup')`)
  - [ ] **CRITICAL**: playwright.config.ts uses storage state for auth sharing (`storageState: 'playwright/.auth/user.json'`)
  - [ ] playwright.config.ts has `ignoreHTTPSErrors: true` for stage environment
  - [ ] **CRITICAL**: HCC_ENV_URL does NOT appear in playwright.config.* (use PLAYWRIGHT_BASE_URL instead)
  - [ ] **CRITICAL**: HCC_ENV_URL does NOT appear in test files (grep -r "HCC_ENV_URL" playwright/ should return nothing)
...
- [ ] E2E credentials configured (choose ONE approach):
  - **ExternalSecret approach (recommended):**
    ...
    - [ ] All 4 required secret keys configured in ExternalSecret
    - [ ] Added to kustomization.yaml in konflux-release-data
    - [ ] Merge request submitted and approved
  - **OR Direct Kubernetes secret (via Konflux UI):**
    - [ ] Secret created via Konflux UI with name `<app-name>-credentials-secret`
    - [ ] Secret type: Opaque
    - [ ] All 4 required secret keys added: e2e-user, e2e-password, e2e-hcc-env-url, e2e-stage-actual-hostname
    - [ ] **Note:** Manual rotation responsibility - no auto-rotation from Vault
- [ ] Pipeline references correct credentials secret name (`<app-name>-credentials-secret`)
...
- **frontend-starter-app**: Reference for Playwright configuration
  ...
  - Demonstrates proper configuration of `@redhat-cloud-services/playwright-test-auth`
  - Includes single worker setup, auth state sharing, and CI-specific settings
