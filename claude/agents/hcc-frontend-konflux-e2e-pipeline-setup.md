...
9. **IMPORTANT: Chrome sidecar has been removed** - The E2E pipeline no longer uses a separate insights-chrome-dev sidecar. All non-app routes now proxy directly to the actual stage environment. This greatly simplifies the proxy ConfigMap.
10. **IMPORTANT: App sidecar ConfigMap no longer needed** - The shared pipeline has been updated so most repositories no longer require a separate ConfigMap for the test application sidecar. Only the frontend-developer-proxy ConfigMap is needed in most cases.
...
spec:
  ...
  params:
    ...
    - name: test-app-port
      value: "8000"  # Port for app assets

    # NOTE: chrome-port parameter has been removed
...
**Step 7: Generate Proxy ConfigMap Using Plumber**
...
**7b. Run Plumber to Generate Proxy ConfigMap**
...
**Note:** If your repository has special requirements and still needs an app-caddy-config ConfigMap, you can include the `--app-configmap-name` flag. Consult with the Platform Experience team if unsure.

**7c. Verify Generated ConfigMap**
...
Plumber will create a YAML file in your current directory:
...
**7d. Submit ConfigMap to konflux-release-data**
...
**CRITICAL:** The E2E pipeline installs dependencies during the workspace setup phase. If @playwright/test is not listed in package.json dependencies or devDependencies, it won't be installed, and the pipeline will fail when trying to run the tests. Ensure the Playwright image version matches your package.json version.

**Common Issue 8: Chrome Sidecar Removal Migration**
...
3. Konflux Migration Phase:
   ...
   - Submit ConfigMap to konflux-release-data via MR
   - Set up Vault secrets and ExternalSecret
   - Test with a PR
...
**IMPORTANT:** The shared pipeline has been updated. Most repositories now only need the proxy ConfigMap. The app-caddy-config ConfigMap is no longer required in most cases.
...
3. Run Plumber:
   ...
   Note: Only include --app-configmap-name if your repository has special requirements.
   Consult with Platform Experience team if unsure.
...
5. Submit to konflux-release-data:
   ...
   - Copy generated YAML to tenants-config/cluster/$YOUR_CLUSTER/tenants/$TENANT_NAMESPACE/ directory
   - Add to kustomization.yaml
   - Create branch and commit
   - Open merge request
...
- [ ] Required E2E pipeline parameters are configured:
  ...
  - [ ] `frontend-proxy-routes-configmap` - ConfigMap name for proxy routes (simplified routing to app port, all else proxies to stage)
  - [ ] `e2e-tests-script` - Script to run Playwright tests
